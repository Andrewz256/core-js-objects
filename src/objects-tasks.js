...
function mergeObjects(objects) {
  const newObject = Object.entries(Object.assign(...objects));
...
function makeWord(lettersObject) {
  const newObject = Object.keys(lettersObject);
  const positionObject = Object.values(lettersObject);
  const newArray = [];
  for (let i = 0; i < newObject.length; i += 1) {
    positionObject[i].forEach((k) => {
      newArray[k] = newObject[i];
    });
  }
